...
exports.run = async (client, message, args) => {
    ...
    .addField("Kullanıcı Information/Bilgi",` 
    
    \`ID:\` ${message.author.id} 
    \`Rollerin:\` ${message.member.roles.cache.size >= 5 ? "Roller çok fazla" : message.member.roles.cache.map(role => role.toString())}
    \`Kullanıcı Adın:\` ${message.member.displayName}
    `)
    .addField("Ses Etkinliği", `
    son Aktivite: ${new Date(voiceData.activity).toLocaleDateString()}

    ** **${voiceList}
    `)
    .addField("Mesaj Etkinliği", `
